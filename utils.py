...
def save_checkpoint(image_model, text_model, text_subnet, subnet_optimizer, optimizer, scaler, epoch, best_test_loss, avg_test_loss, test_auc, checkpoint_dir, name):
	checkpoint = {
		'epoch': epoch + 1,
		'imaget_model_state_dict': image_model.state_dict(),
		'text_model_state_dict': text_model.state_dict(),
		'text_subnet_state_dict': text_subnet.state_dict(),
		'subnet_optimizer_state_dict': subnet_optimizer.state_dict(),
...
def load_checkpoint(checkpoint_dir=None, image_model=None, text_model=None, text_subnet=None, subnet_optimizer=None, optimizer=None, scaler=None):
	if checkpoint_dir:
		checkpoint = torch.load(checkpoint_dir, map_location='cpu')
	if image_model:
		image_model.load_state_dict(checkpoint['imaget_model_state_dict'])
	if text_model:
		text_model.load_state_dict(checkpoint['text_model_state_dict'])
	if text_subnet:
		text_subnet.load_state_dict(checkpoint['text_subnet_state_dict'])
	if subnet_optimizer:
		subnet_optimizer.load_state_dict(checkpoint['subnet_optimizer_state_dict'])
	if optimizer:
